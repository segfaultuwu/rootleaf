...
pub fn read_file(path: &str) -> Result<&'static [u8], VfsError> {
    ...
    let (block_size, inode_size, inode_table_block) =
        parse_superblock(data).ok_or(VfsError::InvalidDisk)?;

    // read root inode (2)
    let (_mode, _size, root_blocks) =
        read_inode(2, block_size, inode_size, inode_table_block).ok_or(VfsError::InvalidDisk)?;
    ...
    for &b in root_blocks.iter() {
        ...
        if found_inode.is_some() {
            break;
        }
    }

    let inode_no = found_inode.ok_or(VfsError::NotFound)?;
    let (_mode, size, blocks) = read_inode(inode_no, block_size, inode_size, inode_table_block)
        .ok_or(VfsError::InvalidDisk)?;

    if size as usize > FILE_READ_BUFFER_SIZE {
        return Err(VfsError::InvalidDisk);
    }
    ...
    for &b in blocks.iter() {
        ...
        if !read_bytes(off, &mut dest[written..written + to_copy]) {
            return Err(VfsError::InvalidDisk);
        }
        written += to_copy;
        if written >= dest.len() {
